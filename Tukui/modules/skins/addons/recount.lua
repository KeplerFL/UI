...
for i = 1, getn(elements) do
	...
	if frame then
		T.set_anim(frame, true, -600, 0, 0.6)
		SkinFrame(frame)
	end
end
...
-- setup the position
if C.general.recountpos then
	local xOfs, yOfs = TukuiTabsLeftBackground:GetCenter()
	--Recount.RestoreMainWindowPosition(xOfs+TukuiTabsLeftBackground:GetWidth()/2+50, yOfs, 200, 200)
	Recount.db.profile.MainWindow.Position.x=xOfs+TukuiTabsLeftBackground:GetWidth()/2+50
	Recount.db.profile.MainWindow.Position.y=yOfs
	Recount.db.profile.MainWindow.Position.w=200
	Recount.db.profile.MainWindow.Position.h=200
	...
	-- FullRefreshMainWindow   ResizeMainWindow
