...
if C["unitframes"].layout ~= 4 then return end
------------------------------------------------------------------------
--	local variables
------------------------------------------------------------------------

local font1 = C["media"].uffont_duf
...
local function Shared(self, unit)
	...
	if (unit == "player" or unit == "target") then
		...
		if T.lowversion then
			panel:CreatePanel("Default", 200, 21, "BOTTOM", self, "BOTTOM", 0, 0)
		else
			panel:CreatePanel("Default", 200, 21, "BOTTOM", self, "BOTTOM", 0, 0)
		end
		...
		health.value = T.SetFontString(health, font1, 10, "MONOCHROMEOUTLINE")
		health.value:Point("BOTTOMLEFT", panel, "BOTTOMLEFT", 2, 2)
		...
		power.value:Point("BOTTOMRIGHT", self.panel, "BOTTOMRIGHT", -2, 2)
		...
		panel:Show()
		panel:Height(15)
		panel:Point("TOPLEFT", power, "BOTTOMLEFT", -2, -6)
		panel:Point("TOPRIGHT", power, "BOTTOMRIGHT", 2, -6)
		
		-- portraits
		if (C["unitframes"].charportrait == true) then
			local portrait = CreateFrame("PlayerModel", self:GetName().."_Portrait", self)
			portrait:SetFrameLevel(8)
			portrait:SetHeight(self.Health:GetHeight() + self.Power:GetHeight()+panel:GetHeight()+12)
			portrait:SetWidth(portrait:GetHeight())
			portrait:SetAlpha(1)
			if unit == "player" then
				portrait:SetPoint("BOTTOMLEFT", panel, "BOTTOMLEFT", -portrait:GetWidth()-9,2)
			elseif unit == "target" then
				portrait:SetPoint("BOTTOMRIGHT", panel, "BOTTOMRIGHT", portrait:GetWidth()+9,2)
			end
		...
		if (unit == "player") then
			...
			if T.level ~= MAX_PLAYER_LEVEL then
				...
				Experience:Width(TukuiMinimap:GetWidth())
				Experience:Height(5)
				Experience:Point("TOP", TukuiMinimap, "BOTTOM", 0, -5)
			...
			if T.level == MAX_PLAYER_LEVEL then
				local Reputation = CreateFrame("StatusBar", self:GetName().."_Reputation", TukuiMinimap)
				...
				Reputation:Width(TukuiMinimap:GetWidth())
				Reputation:Height(5)
				Reputation:Point("TOP", TukuiMinimap, "BOTTOM", 0, -5)
		...
		if (unit == "target" and C["unitframes"].targetauras) then
			...
			buffs:SetPoint("BOTTOMLEFT", self.Health, "TOPLEFT", 0, 15)
		...
		if (C["unitframes"].unitcastbar == true) then
			...
			if (targetClass ~= nil) then
				ChatFrame1:AddMessage(targetClass)
				castbar:SetStatusBarColor(unpack(castbarcolor[targetClass]))
			else
				castbar:SetStatusBarColor(unpack(castbarcolor["MAGE"]))
			end
			--castbar:SetStatusBarColor(unpack(castbarcolor["MAGE"]))
			castbar.SetStatusBarColor = function() return end
			if unit == "player" then
				castbar:Point("TOPLEFT", TukuiInfoBottom, 2, -2)
			castbar:Point("BOTTOMRIGHT", TukuiInfoBottom, -2, 2)
			elseif unit == "target" then
				castbar:SetFrameLevel(6)
	...
	if (unit == "targettarget") then
		...
		self.bg:Point("BOTTOMRIGHT", self.Health, "BOTTOMRIGHT", 2, -2)
	end
