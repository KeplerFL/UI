...
RSA_Reminder = CreateFrame("Frame", "RSA:R") -- Reminder Frame
local function Reminder(self, elapsed) -- Reminder Function
	...
	if RSA.db.profile.Reminders.EnableInSpec1 == true then -- Enable in primary spec?
		if GetActiveTalentGroup("player") == 1 then -- Are we in that spec?
			if RSA.db.profile.Reminders.RemindChannels.Chat == true then
				RSA_Print_Self(ReminderSpell .. L[" is Missing!"])
			end
			if RSA.db.profile.Reminders.RemindChannels.RaidWarn == true then
				RSA_Print_Self_RW(ReminderSpell .. L[" is Missing!"])
			end
		end
	end
	if RSA.db.profile.Reminders.EnableInSpec2 == true then -- Enable in secondary spec?
		if GetActiveTalentGroup("player") == 2 then -- Are we in that spec?
			if RSA.db.profile.Reminders.RemindChannels.Chat == true then
				RSA_Print_Self(ReminderSpell .. L[" is Missing!"])
			end
			if RSA.db.profile.Reminders.RemindChannels.RaidWarn == true then
...
RSA_Monitor = CreateFrame("Frame", "RSA:M") -- Monitor Frame
...
RSA_Reload_Link("Reminders Module disabled, you should ", " your UI.") -- Send Message to remind the user to reload UI, with a reload hyperlink.
RSA.db.profile.Modules.Reminders_Loaded = false
RSA_Monitor:SetScript("OnUpdate", nil)
RSA_Reminder:SetScript("OnUpdate", nil)
end -- End OnDisable